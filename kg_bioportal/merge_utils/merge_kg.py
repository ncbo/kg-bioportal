...
def update_merge_config(
    yaml_file: str, merge_all: bool, include_only: list, exclude: list
) -> None:
    """Update the merge config YAML with values from runtime params.
    ...
    if len(include_only) > 0:
        onto_dirs = [
            dirname
            for dirname in os.listdir(ONTO_DATA_PATH)
            if os.path.isdir(os.path.join(ONTO_DATA_PATH, dirname))
            and dirname in include_only
        ]
    elif len(exclude) > 0:
        onto_dirs = [
            dirname
    ...
    for dirname in onto_dirs:
        ...
        onto_paths[dirname] = [
            os.path.join(this_path, filename)
    ...
    with open(yaml_file) as yaml_file_in:
        ...
        for onto in onto_paths:
            updated_config["merged_graph"]["source"][f"s{i}"] = {
                "name": onto,
                "input": {"format": "tsv", "filename": onto_paths[onto]},
            }
            i = i + 1

    with open(yaml_file, "w") as updated_yaml_file:
        updated_yaml_file.write(
            yaml.dump(updated_config, default_flow_style=False, sort_keys=False)
        )
    ...
    with tarfile.open(merge_graph_path) as intar:
        ...
        for graph_file in graph_files:
            ...
            graph_file_paths.append(
                os.path.join(os.path.dirname(merge_graph_path), graph_file)
            )
